    ...
    // const db = require('./lib/model/db')
      ...
      return sequelize.models.Leave.findAll()
        .then( (args) => {
          ...
          console.log('in the then above while loop')
          while(promiseArray.length < args.length) {
            console.log('inside the while loop')
              promiseArray.push(sequelize.models.User.findOne({
                attributes: ['name'],
                where: {
                  id: args[index].dataValues.userId
                }
                })
            )
            index++
          ...
          console.log(promiseArray.length)
            if(promiseArray.length === args.length) {
              console.log('promiseArray is full')
              return Promise.all(promiseArray)
                .then( results => {
                  console.log('in the final then')
                  return results
                })
          }
        })
